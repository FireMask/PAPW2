    ...
    <div class="content">
        ...
        <div class="row">
            <transition-group name="translate-fade" tag="div" mode="out-in">
                <div :key="dato.idMoneda" v-for="dato in datosMostrados" class="col-md-12" v-on:click="seleccionarDato(dato);">
                    <view-moneda :moneda="dato"></view-moneda>
                </div>
            </transition-group>
        </div>
        ...
        computed: {
            datosFiltrados: function () {
                ...
                var datos = this.datos.filter(function(item){
                    return item.nombre.toLowerCase().indexOf(self.busqueda) !== -1 || item.simbolo.toLowerCase().indexOf(self.busqueda) !== -1;
                });
                this.paginaActual = 0;
                return datos;
            },
