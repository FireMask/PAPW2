    ...
    <div class="content">
        ...
        <div class="row">
            <transition-group name="translate-fade" tag="div" mode="out-in">
                <div :key="dato.idProducto" v-for="dato in datosMostrados" class="col-md-12" v-on:click="seleccionarDato(dato);">
                    <view-producto :producto="dato"></view-producto>
                </div>
            </transition-group>
        </div>
        ...
        computed: {
            datosFiltrados: function () {
                ...
                var datos = this.datos.filter(function(item){
                    return item.descripcion.toLowerCase().indexOf(self.busqueda) !== -1 || item.modelo.toLowerCase().indexOf(self.busqueda) !== -1 || item.codigo.toLowerCase().indexOf(self.busqueda) !== -1;
                });
                this.paginaActual = 0;
                return datos;
            },
